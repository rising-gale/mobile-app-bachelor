...
const AssessmentPage: React.FC = () => {
  ...
  const handleCancelAndBack = async () => {
    try {
      // Проверяем, есть ли локальный путь к файлу, и удаляем его из кэша устройства
      if (imageUri) {
        await FileSystem.deleteAsync(imageUri, { idempotent: true });
      }
    } catch (error) {
      console.log("Ошибка очистки кэша фото:", error);
    } finally {
      // Возвращаемся назад на камеру
          ...
          <View className="flex-1 justify-center items-center px-6">
            ...
            <Text className="text-red-400 text-center font-semibold mt-4 text-base">
              {(error as any)?.data?.detail ||
                "Не вдалося завантажити дані про автомобіль"}
            </Text>
            <TouchableOpacity
              onPress={handleCancelAndBack}
              className="mt-6 bg-[#9B907B] px-6 py-2.5 rounded-xl"
            >
              <Text className="text-[#181c24] font-bold">
                Повернутись до камери
              </Text>
            </TouchableOpacity>
          </View>
        ) : info ? (
            ...
            <View className="items-center my-4">
              <View className="bg-white border-2 border-slate-800 rounded-lg px-6 py-2 shadow-md flex-row items-center space-x-3">
                <View className="bg-blue-700 w-4 h-8 justify-between items-center rounded-sm py-0.5">
                  <Text className="text-[6px] text-yellow-400 font-bold">
                    UA
                  </Text>
                </View>
              ...
              {imageUri ? (
                ...
                <View className="w-full h-52 bg-[#252d3d] rounded-xl justify-center items-center">
                  ...
                  <Text className="text-slate-500 text-xs mt-2">
                    Фотографія автомобіля відсутня
                  </Text>
                </View>
              )}
            </View>
